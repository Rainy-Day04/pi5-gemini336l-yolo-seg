...
- **pip 开始下载 `nvidia-cudnn-cu13` / `cuda-toolkit`**：立即取消并 `git pull`。Pi 5 没有 NVIDIA GPU；新版安装器会固定官方 CPU-only PyTorch、清理专用 venv 中残留的 `nvidia-*-cu13` 包，并验证 `torch.version.cuda is None`。
